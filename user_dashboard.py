...
def render_data_pipeline_page():
    try:
        ...
        if uploaded_file is not None:
            file_content_bytes_for_cache = uploaded_file.getvalue()
            ...
            with st.spinner("Processing data..."):
                try:
                    ...
                    st.error(f"Error loading data from uploaded file: {e}")
                ...
                if df is not None and not df.empty:
                    ...
                    if cleaning_summary is None:
                        cleaning_summary = {
                            "before": {"rows": df.shape[0] if df is not None else "N/A", "missing_values": {}},
                            "after": {"rows": df_cleaned.shape[0] if df_cleaned is not None else "N/A"},
                    ...
                    if missing_before:
                        missing_values_df = pd.DataFrame(list(missing_before.items()), columns=['Column', 'Missing Count'])
                        st.dataframe(missing_values_df)
                    else:
                        st.info("No missing values data in summary or no missing values found.")
                    ...
                    if st.button("Run Clustering Analysis", key="run_clustering_button"):
                        if not selected_algorithms:
                            ...
                            with st.spinner("Running clustering algorithms..."):
                                try:
                                    X_processed, preprocessor_details = processor.preprocess_data(df_cleaned.copy())
                                except Exception as e:
                                    st.error(f"Error during data preprocessing: {e}")
                                    logger.error(f"Exception in processor.preprocess_data: {e}", exc_info=True)
                                    X_processed = None
                                ...
                                for algorithm in selected_algorithms:
                                    ...
                                    preprocessor_details_str = json.dumps(preprocessor_details, sort_keys=True) if isinstance(preprocessor_details, dict) else str(preprocessor_details or "None")
                                    ...
                                    if cached_results:
                                        results[algorithm] = cached_results
                                    else:
                                        try:
                                            ...
                                            if labels is not None and model is not None:
                                                ...
                                                # Ensure visualize_clusters sets a colorful template on the figure
                                                ...
                                                results[algorithm] = {"labels": labels, "metrics": metrics, "visualization": fig_viz, "model": model}
                                                processor.save_to_cache(cache_key, results[algorithm])
                                            else: results[algorithm] = None 
                                        except Exception as e:
                                ...
                                if any(res for res in results.values() if res is not None):
                                    for algo, res_item in results.items():
                                    ...
                                    if metrics_data:
                                        ...
                                        def create_metric_bar_chart(m_df, y_col_df, t_chart):
                                            if y_col_df in m_df.columns and not m_df[y_col_df].isnull().all():
                                                st.plotly_chart(px.bar(m_df,x="Algorithm",y=y_col_df,color="Algorithm",title=t_chart), use_container_width=True, key=f"chart_{y_col_df.lower().replace(' ','_')}")
                                        ...
                                        for score_col, score_title in chart_cols_titles:
                                            create_metric_bar_chart(metrics_df, score_col, score_title)

                                        st.subheader("Algorithm Performance Radar Chart")
                                        all_algorithms_radar = metrics_df['Algorithm'].unique().tolist()
                                        selected_algorithms_for_radar = st.multiselect("Select algorithms for Radar Chart:", all_algorithms_radar, default=all_algorithms_radar, key="radar_algo_select")
                                        if selected_algorithms_for_radar:
                                            try:
                                                # Ensure create_radar_chart sets a colorful template
                                                radar_fig = processor.create_radar_chart(metrics_df, selected_algorithms_for_radar)
                                                if radar_fig: st.plotly_chart(radar_fig, use_container_width=True, key="radar_chart_comparison")
                                    ...
                                    viz_count = 0
                                    for algo, res_item_viz in results.items():
                                        if res_item_viz and res_item_viz.get("visualization"):
                                            st.write(f"### {algo}"); st.plotly_chart(res_item_viz["visualization"], use_container_width=True, key=f"viz_{algo}"); viz_count+=1
                                    if viz_count == 0: st.info("No visualizations available.")
                                    
                                    current_username_log = auth.get_username()
                                    if current_username_log and save_analysis_run:
                                        try: 
                                            processor.log_dataset_processing(current_username_log, uploaded_file.name, df.shape if df is not None else "N/A", cleaning_summary, [alg for alg, r_alg in results.items() if r_alg])
                                            analysis_run_data = {
                                                "file_name": uploaded_file.name, "original_shape": df.shape if df is not None else "N/A",
                                                "cleaning_summary": cleaning_summary, "preprocessor_details": preprocessor_details,
                                                "algorithms_run": selected_algorithms,
                                                "results": { algo_r: {
                                                        "metrics": r_item.get("metrics"),
                                                        "visualization_json": r_item.get("visualization").to_json() if r_item.get("visualization") else None,
                                                    } for algo_r, r_item in results.items() if r_item },
                                                "metrics_summary_df_json": metrics_df.to_json(orient='split') if 'metrics_df' in locals() and metrics_df is not None else None }
                                    ...
                                    if 'metrics_df' in locals() and metrics_df is not None and not metrics_df.empty:
                                        ...
                                        st.download_button(label="Download Metrics Table (CSV)", data=csv_metrics, file_name='clustering_metrics.csv', mime='text/csv', key='download_metrics_csv')

                                        if results:
                                            download_algo_options = [algo_dl for algo_dl in selected_algorithms if algo_dl in results and results[algo_dl] and 'labels' in results[algo_dl]]
                                            if download_algo_options:
                                                selected_algo_for_download = st.selectbox("Select algorithm for downloading labeled data:", options=download_algo_options, key='select_algo_download_labels')
                                                if selected_algo_for_download and selected_algo_for_download in results:
                                                    df_cleaned_with_labels = df_cleaned.copy()
                                                    labels_for_df = results[selected_algo_for_download]['labels']
                                                    if len(labels_for_df) == len(df_cleaned_with_labels):
                                                        df_cleaned_with_labels[f'{selected_algo_for_download}_Cluster'] = labels_for_df
                                                        csv_labeled_data = df_cleaned_with_labels.to_csv(index=False).encode('utf-8')
                                                        st.download_button(
                                                            ...
                                                            mime='text/csv',
                                                            key=f'download_labeled_data_csv_{selected_algo_for_download}'
                                                        )
                                                    else:
                                                        st.warning(f"Could not prepare labeled data for {selected_algo_for_download}: Label count mismatch.")
                                            else:
                                                st.info("No algorithm results with labels available for download.")

                                        # Full interactive report (HTML)
                                        st.markdown("--- \n**Interactive Report (HTML)**")
                                        metrics_table_html_content = metrics_df.to_html(index=False, classes="dataframe")
                                        visualizations_html_content_parts = []
                                        for algo_html, res_item_html in results.items():
                                            if res_item_html and res_item_html.get("visualization"):
                                                try:
                                                    fig_for_html = res_item_html["visualization"]
                                                    # The figure itself should have a colorful template (e.g., 'plotly') set by the DataProcessor
...
<body>
    <div class="container">
        ...
        {visualizations_html_final_content}
                                        ...
                                        st.download_button(label="Download Interactive Report (HTML)", data=full_report_html_bytes, file_name='clustering_report.html', mime='text/html', key='download_html_report')

                                        # Static PDF report feature has been removed.

                                else: st.error("No clustering results were successfully generated to create reports.")
                elif df is None: st.error("Failed to load data from uploaded file.")
                elif df.empty: st.error("Uploaded data is empty after loading.")
        else: st.info("Please upload a CSV or Excel file to begin analysis.")
    except Exception as e:
...
def render_analysis_history_page():
    ...
    if selected_run_display and get_analysis_run_details:
        ...
        col_detail1, col_detail2 = st.columns([0.8, 0.2])
        with col_detail1:
            run_details = get_analysis_run_details(selected_run_id)
        with col_detail2:
            st.markdown("<br>", unsafe_allow_html=True) # Add some vertical space
            if st.button("Delete Selected Analysis", key="delete_selected_analysis_button"):
                if st.session_state.get("username") and selected_run_id:
                    delete_user_analysis_run(st.session_state.username, selected_run_id)
                    st.success(f"Analysis run {selected_run_id} deleted!")
                    st.rerun()
        ...
        if run_details:
            ...
            if run_details.get('metrics_summary_df_json'):
                ...
                try:
                    ...
                    # Add download button for metrics table as CSV
                    ...
                    st.download_button(
                        ...
                        mime="text/csv",
                        key=f"download_metrics_csv_{selected_run_id}"
                    )
            ...
            for algo_hist, res_item_hist in results_history.items():
                if res_item_hist and res_item_hist.get("visualization_json"):
                    ...
                    try:
                        ...
                        # The figure JSON should ideally contain layout including template.
                        # If not, a default colorful template is desirable.
                        # fig_history.update_layout(template="plotly") # Consider if needed
            ...
            if st.button("Download HTML Report", key=f"download_html_report_{selected_run_id}"):
                # This is a simplified mock. In a real app, you'd generate a full HTML report.
                # For now, we'll just create a simple HTML string.
                html_content = f"""
                <!DOCTYPE html>
                <html>
                <head><title>Analysis Report for {run_details.get('file_name', 'N/A')}</title></head>
                <body>
                    <h1>Analysis Report</h1>
                    <p>Timestamp: {run_details['timestamp']}</p>
                    <p>File: {run_details.get('file_name', 'N/A')}</p>
                    <h2>Metrics Summary</h2>
                    {metrics_df_history.to_html(index=False) if 'metrics_df_history' in locals() else "<p>No metrics data.</p>"}
                    <h2>Preprocessor Details</h2>
                    <pre>{json.dumps(run_details.get('preprocessor_details', {}), indent=2)}</pre>
                    <h2>Algorithms Run</h2>
                    <p>{', '.join(run_details.get('algorithms_run', []))}</p>
                    <!-- Visualizations would be embedded here in a real report -->
                </body>
                </html>
                """
                st.download_button(
                    ...
                    file_name=f"analysis_report_{selected_run_id}.html",
                    mime="text/html",
                    key=f"download_html_{selected_run_id}"
                )

        elif not get_analysis_run_details:
            st.error("Function to get analysis run details is not available.")
        else: st.error("Could not retrieve details for selected run.")
...
def render_user_dashboard():
    try:
        ...
        page_options = ["Profile", "Data Pipeline", "Analysis History"]
...
class MockAuthentication:
    ...
    def logout(self):
        ...
        st.session_state.dashboard_page = "Data Pipeline"
        for k in ['username','full_name','email','role']: st.session_state.pop(k, None)
        st.success("Mock: Logged out."); st.rerun()

class MockDataProcessor:
    def load_data(self, file): return pd.DataFrame({'A':[1,2,1,2,1]*4,'B':[3,4,3,4,4]*4,'C':[5,6,5,6,5]*4,'D':[7,8,7,8,8]*4, 'E':[1,1,2,2,1]*4}) if file else None
    def clean_data(self, df): return df.copy(), {"before":{"rows":df.shape[0],"missing_values":{"E":0}},"after":{"rows":df.shape[0]},"duplicates_removed":0}
    def preprocess_data(self, df): 
        ...
        if numeric_cols.empty:
             return pd.DataFrame().to_numpy(), {"error": "No numeric columns to process"}
    ...
    def apply_clustering(self, X, algo, k_cfg): 
        ...
        if X.shape[0] == 0: return [], {"name":algo,"params":{"n_clusters":k_val}, "error": "Empty data for clustering"}
        return [i%k_val for i in range(X.shape[0])], {"name":algo,"params":{"n_clusters":k_val}}
        
    def evaluate_clustering(self, X, lbls): 
        if not lbls or X is None or X.shape[0] < 2 or len(set(lbls)) < 2:
            return {"silhouette_score":None,"calinski_harabasz_score":None,"davies_bouldin_score":None,"n_clusters_found":len(set(l for l in lbls if l!=-1))}
        return {"silhouette_score":0.55,"calinski_harabasz_score":150.0,"davies_bouldin_score":0.8,"n_clusters_found":len(set(l for l in lbls if l!=-1))}
    
    def visualize_clusters(self, X, lbls, algo):
        title_suffix = " (Mock Data)"
        # IMPORTANT: Apply a colorful template to the figure object
        template_to_use = "plotly" # Examples: "plotly", "plotly_dark", "ggplot2", "seaborn", "plotly_white"

        if X is None or X.shape[0] == 0:
            fig = go.Figure()
            fig.update_layout(title=f"{algo} Clusters{title_suffix} - No Data", template=template_to_use)
            return fig

        if X.shape[1] >= 2:
            fig = px.scatter(x=X[:, 0], y=X[:, 1], color=[str(l) for l in lbls], title=f"{algo} Clusters{title_suffix}")
        elif X.shape[1] == 1:
            fig = px.scatter(x=range(len(X)), y=X[:, 0], color=[str(l) for l in lbls], title=f"{algo} Clusters (1D){title_suffix}")
        else: 
            fig = go.Figure()
            fig.update_layout(title=f"{algo} Clusters{title_suffix} - Not enough dimensions", template=template_to_use)
            return fig
    ...
    def load_from_cache(self, key): logger.info(f"Mock: Cache load attempt for {key}"); return None
    def save_to_cache(self, key, data): logger.info(f"Mock: Cache save for {key}")
    
    def create_radar_chart(self, df_m, selected_algorithms_for_radar_input):
        if not selected_algorithms_for_radar_input: return None
        ...
        if not df_m_filtered.empty and "Algorithm" in df_m_filtered.columns:
            ...
            df_r.dropna(subset=available_metric_cols, how='all', inplace=True)
            if df_r.empty: return None

            df_r_melted = df_r.melt(id_vars="Algorithm",var_name="Metric",value_name="Score")
            df_r_melted.dropna(subset=['Score'], inplace=True)
                ...
                for metric_name in df_r_melted["Metric"].unique():
                    ...
                    if not scores_for_metric.empty:
                        ...
                        if pd.notna(min_v) and pd.notna(max_v):
                            if max_v > min_v : df_r_melted.loc[slc,"Score"] = (scores_for_metric - min_v) / (max_v - min_v)
                            elif max_v == min_v : df_r_melted.loc[slc,"Score"] = 0.5 if min_v !=0 else 0.0
                            else: df_r_melted.loc[slc,"Score"] = 0.0
                        else: df_r_melted.loc[slc,"Score"] = 0.0
                    else: df_r_melted.loc[slc,"Score"] = 0.0
                # IMPORTANT: Apply a colorful template
                fig = px.line_polar(df_r_melted,r="Score",theta="Metric",color="Algorithm",line_close=True,title="Algorithm Radar (Mock, Normalized)")
                fig.update_layout(template="plotly") # Ensures colors
                return fig
        return None
...
_mock_analysis_history_store = {}

def mock_get_user_analysis_history(username):
    logger.info(f"Mock: Getting history for {username}")
    if username not in _mock_analysis_history_store:
        base_time = datetime.datetime.now()
        mock_fig_processor = MockDataProcessor()
        mock_fig_data = mock_fig_processor.visualize_clusters(
            pd.DataFrame({'x':[1,2,3],'y':[3,1,2]}).to_numpy(),
            [0,1,0],
            "KMeans_Hist_Mock"
        )
        _mock_analysis_history_store[username] = [
            {"_id": "mock_run_1", "timestamp": (base_time - datetime.timedelta(days=1)).strftime("%Y-%m-%d %H:%M:%S"), "file_name": "hist_data1.csv",
             "original_shape": [10,2], "cleaning_summary":{}, "preprocessor_details":{}, "algorithms_run":["KMeans_Hist_Mock"],
             "metrics_summary_df_json": pd.DataFrame({"Algorithm":["KMeans_Hist_Mock"],"Silhouette Score":[0.5]}).to_json(orient='split'),
             "results": {"KMeans_Hist_Mock": {"metrics":{"silhouette_score":0.5}, "visualization_json": mock_fig_data.to_json() if mock_fig_data else None }}
             },
            {"_id": "mock_run_2", "timestamp": (base_time - datetime.timedelta(hours=12)).strftime("%Y-%m-%d %H:%M:%S"), "file_name": "hist_data2.csv",
             "original_shape": [20,3], "cleaning_summary":{}, "preprocessor_details":{}, "algorithms_run":["DBSCAN_Hist_Mock"],
             "metrics_summary_df_json": pd.DataFrame({"Algorithm":["DBSCAN_Hist_Mock"],"Silhouette Score":[0.6]}).to_json(orient='split'),
             "results": {"DBSCAN_Hist_Mock": {"metrics":{"silhouette_score":0.6}, "visualization_json": mock_fig_data.to_json() if mock_fig_data else None }}
             }
        ]
...
def mock_get_analysis_run_details(run_id):
    ...
    for username in _mock_analysis_history_store:
        for run in _mock_analysis_history_store[username]:
            if run['_id'] == run_id:
                return run
    return None
...
if __name__ == "__main__":
    ...
    # This is a bit simplified; a robust check would involve checking actual types
    if Authentication is None or DataProcessor is None or \
       update_user is None or save_analysis_run is None or \
       get_user_analysis_history is None or get_analysis_run_details is None:
    ...
    # Ensure these functions are always defined, either by actual import or mock
    if 'clear_user_analysis_history' not in globals() or clear_user_analysis_history is None:
        clear_user_analysis_history = mock_clear_user_analysis_history
    if 'delete_user_analysis_run' not in globals() or delete_user_analysis_run is None:
